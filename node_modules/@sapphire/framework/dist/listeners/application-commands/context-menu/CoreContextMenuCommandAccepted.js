...
var _CoreListener = class _CoreListener extends Listener_js.Listener {
  constructor(context) {
    super(context, { event: Events_js.Events.ContextMenuCommandAccepted });
  }
  async run(payload) {
    const { command, context, interaction } = payload;
    const result$1 = await result.Result.fromAsync(async () => {
      this.container.client.emit(Events_js.Events.ContextMenuCommandRun, interaction, command, { ...payload });
      ...
      this.container.client.emit(Events_js.Events.ContextMenuCommandSuccess, { ...payload, result: result2, duration });
      return duration;
    });
    result$1.inspectErr((error) => this.container.client.emit(Events_js.Events.ContextMenuCommandError, error, { ...payload, duration: -1 }));
    this.container.client.emit(Events_js.Events.ContextMenuCommandFinish, interaction, command, {
      ...payload,
