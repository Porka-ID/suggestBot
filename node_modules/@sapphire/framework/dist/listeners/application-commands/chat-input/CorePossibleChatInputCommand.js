...
var _CoreListener = class _CoreListener extends Listener_js.Listener {
  constructor(context) {
    super(context, { event: Events_js.Events.PossibleChatInputCommand });
  }
  run(interaction) {
    ...
    if (!command) {
      client.emit(Events_js.Events.UnknownChatInputCommand, {
        interaction,
    ...
    if (!command.chatInputRun) {
      client.emit(Events_js.Events.CommandDoesNotHaveChatInputCommandHandler, {
        command,
        interaction,
        context: { commandId: interaction.commandId, commandName: interaction.commandName }
      });
    ...
    client.emit(Events_js.Events.PreChatInputCommandRun, {
      command,
      context: { commandId: interaction.commandId, commandName: interaction.commandName },
      interaction
    });
