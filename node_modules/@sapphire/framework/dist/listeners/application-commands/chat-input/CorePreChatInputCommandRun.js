...
var _CoreListener = class _CoreListener extends Listener_js.Listener {
  constructor(context) {
    super(context, { event: Events_js.Events.PreChatInputCommandRun });
  }
  async run(payload) {
    ...
    if (globalResult.isErr()) {
      this.container.client.emit(Events_js.Events.ChatInputCommandDenied, globalResult.unwrapErr(), payload);
      return;
    }
    const localResult = await command.preconditions.chatInputRun(interaction, command, payload);
    if (localResult.isErr()) {
      this.container.client.emit(Events_js.Events.ChatInputCommandDenied, localResult.unwrapErr(), payload);
      return;
    }
    this.container.client.emit(Events_js.Events.ChatInputCommandAccepted, payload);
  }
};
