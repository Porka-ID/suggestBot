...
var _CoreListener = class _CoreListener extends Listener_js.Listener {
  constructor(context) {
    super(context, { event: Events_js.Events.PrefixedMessage });
  }
  run(message, prefix) {
    ...
    if (commandName.length === 0) {
      client.emit(Events_js.Events.UnknownMessageCommandName, { message, prefix, commandPrefix });
      return;
    }
    const command = stores.get("commands").get(client.options.caseInsensitiveCommands ? commandName.toLowerCase() : commandName);
    if (!command) {
      client.emit(Events_js.Events.UnknownMessageCommand, { message, prefix, commandName, commandPrefix });
      return;
    }
    if (!command.messageRun) {
      client.emit(Events_js.Events.CommandDoesNotHaveMessageCommandHandler, { message, prefix, commandPrefix, command });
      return;
    }
    const parameters = spaceIndex === -1 ? "" : prefixLess.substring(spaceIndex + 1).trim();
    client.emit(Events_js.Events.PreMessageCommandRun, {
      message,
      command,
      parameters,
