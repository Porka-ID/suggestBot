...
function resolveDate(parameter, options) {
  ...
  if (Number.isNaN(time)) {
    return result.Result.err(Identifiers_js.Identifiers.ArgumentDateError);
  }
  if (typeof options?.minimum === "number" && time < options.minimum) {
    return result.Result.err(Identifiers_js.Identifiers.ArgumentDateTooEarly);
  }
  if (typeof options?.maximum === "number" && time > options.maximum) {
    return result.Result.err(Identifiers_js.Identifiers.ArgumentDateTooFar);
  }
