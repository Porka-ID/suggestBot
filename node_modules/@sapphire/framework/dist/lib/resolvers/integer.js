...
function resolveInteger(parameter, options) {
  ...
  if (!Number.isInteger(parsed)) {
    return result.Result.err(Identifiers_js.Identifiers.ArgumentIntegerError);
  }
  if (typeof options?.minimum === "number" && parsed < options.minimum) {
    return result.Result.err(Identifiers_js.Identifiers.ArgumentIntegerTooSmall);
  }
  if (typeof options?.maximum === "number" && parsed > options.maximum) {
    return result.Result.err(Identifiers_js.Identifiers.ArgumentIntegerTooLarge);
  }
