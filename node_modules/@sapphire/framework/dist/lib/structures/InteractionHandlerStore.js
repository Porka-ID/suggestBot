...
var _InteractionHandlerStore = class _InteractionHandlerStore extends pieces.Store {
  ...
  async run(interaction) {
    ...
    for (const handler of this.values()) {
      ...
      result$1.match({
        ok: (option) => {
          this.container.client.emit(Events_js.Events.InteractionHandlerParseSuccess, option, { interaction, handler });
          option.match({
            // If the `parse` method returned a `Some` (whatever that `Some`'s value is, it should be handled)
            some: (value) => {
              this.container.client.emit(Events_js.Events.InteractionHandlerParseSome, option, {
                interaction,
                handler,
                value
              });
            ...
            none: () => this.container.client.emit(Events_js.Events.InteractionHandlerParseNone, option, { interaction, handler })
          });
        },
        err: (error) => {
