...
function isTextBasedChannel(channel) {
  if (utilities.isNullish(channel) || isStageChannel(channel))
    return false;
...
function createPartitionedMessageRow(components) {
  ...
  const chunkedButtonComponents = utilities.chunk(messageButtons, 5);
  ...
  return [...messageButtonActionRows, ...selectMenuActionRows].map(
...
var _PaginatedFieldMessageEmbed = class _PaginatedFieldMessageEmbed extends PaginatedMessage {
  ...
  generatePages() {
    ...
    for (let i = 0; i < this.totalPages; i++) {
      ...
      if (utilities.isNullish(template.color))
  ...
  resolveTemplate(template) {
    if (utilities.isFunction(template))
      template = template(new discord_js.EmbedBuilder());
...
var _PaginatedMessageEmbedFields = class _PaginatedMessageEmbedFields extends PaginatedMessage {
  ...
  make() {
    if (!this.items.length)
      throw new Error("The items array is empty.");
    if (this.itemsPerPage > discordUtilities.EmbedLimits.MaximumFields)
      throw new Error(`Pages cannot contain more than ${discordUtilities.EmbedLimits.MaximumFields} fields.`);
    this.totalPages = Math.ceil(this.items.length / this.itemsPerPage);
    this.generatePages();
    return this;
  }
